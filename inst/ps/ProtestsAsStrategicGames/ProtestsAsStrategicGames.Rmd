...
The problem set is available on ShinyApps and in the form of a package hosted via github:

* `GitHub`: <https://github.com/RobertLaetsch/RTutorProtestsAsStrategicGames>
* `ShinyApps`: <https://robertlae.shinyapps.io/ProtestsAsStrategicGames/>
...
Quiz: Hong Kong is set to keep this status...
...
answer.quiz("Special Administrative Region")
```
...
![Umbrella Revolution](umbrella_revolution.webp)
...
Quiz: How many of Hong Kong's 7,4 million inhabitants participated in the largest protest of 2019?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Summer of Uprising")
```
...
Quiz: Students beliefs regarding the benefits and costs of hypothetical protests point towards...

(1) strategic substitutability
(2) strategic complementarity
(3) both strategic substitutability and complementarity
(4) neither strategic substitutability nor complementarity

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Cost and benefits of protests")
```
...
![Experimental Design](Experimental_Design.jpeg){width=80%, height = 80%}
...
```{r "4_2"}
#Fill in the blanks
data<-___("protests_as_strategic_games.dta")
```
...
Quiz: Are the treatment and control group balanced?
...
Quiz: Which attitude is most likely to change when cover is provided?

(1) Unfavorable view of CCP
(2) Consider self Hong Kongese
(3) Support HK independence
(4) Support violence in pursuit of policital rights
(5) none of the above

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Item count technique")
```
...
**Task:** Use `readRDS()` to load `dat.RDS` and save it in `data`
...
Quiz: Subjects in the treatment group...

(1) all updated their beliefs positively
(2) updated their belief towards the true level of planned participation
(3) consistently updated their beliefs negatively

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Density of beliefs")
```
...
Quiz: Do we expect significant updating of beliefs in the control group?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Effect on Beliefs in the Control Group")
```
...
Quiz: Do we observe heterogeneous treatment effects in beliefs in the binned scatterplot?

(1) yes
(2) no
(3) not observable in this graph

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Heterogenous treatment effects")
```
...
Quiz: Is the monotonic treatment effect higher or lower than 0.691?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Monotonic treatment effect")
```
...
Quiz: Compare the regression results to the table above. What has changed?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Robust standard errors")
```
...
Quiz: Does the sensitivity analysis indicate good randomization?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Randomization in our first stage estimates")
```
...
Quiz: If our inferences from Table 3 are sound, the t values of retroactive random assignment are...

(1) evenly distributed
(2) distributed around the t value of the actual assignment
(3) distributed around 0

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("First stage randomization inference")
```

**Task:** Check the chunk to plot the distribution of t statistics.

```{r "5_29",fig.cap=''}
randomization_1ststage_inference <- readRDS("random_1ststage_inference.rds")
...
Quiz: Do the treatment effects on beliefs and participation move in the same direction?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Effect on beliefs vs participation")
```
...
Quiz: This indicates a game of...

(1) strategic complements
(2) strategic substitutes

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Effects on turnout")
```
...
Quiz: Direction adjusted treatment...

(1) caused participation to decrease by 2.713 percent
(2) caused participation to decrease by 2.713 percentage points

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Monotonic treatment on participation")
```
...
```{r "6_10",fig.cap=''}
randomization_reducedform_inference<-readRDS("random_reducedform_inference.rds")
...
Quiz: Select the correct statement.

(1) 6% of individuals with high priors from the control group would be persuaded if treated.
(2) 6% of individuals with high priors would participate when treated, but would not in the absence of treatment.
(3) 6% of individuals who were not already persuaded change their behaviour if treated.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Persuasion Rate")
```
...
Quiz: Do we observe heterogeneity in the treatment effect on participation?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Heterogeneity in treatment")
```
...
Quiz: A one percentage point increase in prior beliefs...

(1) increases the treatment effect by 0.248 percentage points
(2) increases participation by 0.248 percentage points
(3) decreases participation by 0.079 percentage points

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interaction terms")
```
...
Quiz: Can heterogeneity in the treatment effect on participation only be attributed to individual characteristics?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Variation in beliefs")
```
...
Quiz: Is beta1 an unbiased estimator for the causal effect of posterior beliefs on participation?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Causal effect on participation")
```
...
Quiz: Does this violate the exclusion condition?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Exclusion condition")
```
...
Quiz: This means we observe a game of strategic...
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Two-stage estimate")
```
